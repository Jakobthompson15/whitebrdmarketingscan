  ...
  return (
    <div className="w-full max-w-2xl">
      ...
      <div className="text-center mb-8">
        <p className="text-lg text-white font-medium animate-pulse-slow">
          {currentStep}
        </p>
      </div>
      ...
      <div className="space-y-3 max-h-64 overflow-y-auto">
        {updates.slice(-5).map((update, index) => (
          <div key={index} className="flex items-center justify-center space-x-3 text-sm text-gray-400 animate-slide-up">
            <div className={`w-2 h-2 rounded-full ${
              update.progress === progress ? 'bg-[var(--color-data-orange)] animate-pulse' : 'bg-gray-600'
            }`} />
            <span className={update.progress === progress ? 'text-white' : 'text-gray-400'}>
              {update.message}
            </span>
